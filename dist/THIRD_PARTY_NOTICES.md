...
Bernardo Musiello’s contributions are licensed under the root MIT LICENSE. Keep that notice and the applicable notices below when redistributing the project. Personal names and provider trademarks are not granted as branding rights by the code license.
...
The original SIL Open Font License 1.1 notices remain alongside the font files:
...
The PNG illustrations were generated with **GPT Image 2.5 through Higgsfield** for this portfolio, with subsequent transparency cleanup, cropping and resizing. `ARTWORK.json` lists every distributed illustration and its recorded model, quality and dimensions. Generation account/job identifiers are intentionally omitted.

To the extent Bernardo Musiello holds transferable rights in these illustrations, he makes them available under the project’s MIT license for reuse, modification and redistribution. This does not assert exclusive copyright in AI-generated output or grant rights to third-party trademarks. No paid generation service or account is needed to run or redistribute the included site.

## Quotations

The nature quote and Lao Tzu attribution were supplied for the portfolio. The moon quote is displayed without an author. They are quotations rather than original Bernardo copy; replace them if adapting the portfolio’s voice.
